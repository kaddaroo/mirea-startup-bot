...
def _sync_attendance_coins_sync(reward: int) -> int:
    """Award coins exactly once for every reg row marked attended=1."""
    awarded = 0
...
async def sync_attendance_coins(reward: int) -> int:
    return await asyncio.to_thread(_sync_attendance_coins_sync, reward)
...
def _save_feedback_sync(telegram_id: int, event_id: int, rating: int, comment: str | None = None):
    with db_cursor() as (_connection, cursor):
        ...
        cursor.execute(
            """
            INSERT INTO event_feedback (id_event, id_user, rating, comment)
            VALUES (%s, %s, %s, %s)
            ON DUPLICATE KEY UPDATE
                ...
                updated_at = CURRENT_TIMESTAMP
            """,
            (event_id, user["id"], rating, comment),
        )
        return True


async def save_feedback(telegram_id: int, event_id: int, rating: int, comment: str | None = None):
    return await asyncio.to_thread(_save_feedback_sync, telegram_id, event_id, rating, comment)


def _set_feedback_comment_sync(telegram_id: int, event_id: int, comment: str):
    with db_cursor() as (_connection, cursor):
        cursor.execute(
            """
            UPDATE event_feedback ef
            JOIN users u ON u.id = ef.id_user
            SET ef.comment = %s, ef.updated_at = CURRENT_TIMESTAMP
            WHERE u.telegram_id = %s AND ef.id_event = %s
            """,
            (comment, telegram_id, event_id),
        )
        return cursor.rowcount > 0


async def set_feedback_comment(telegram_id: int, event_id: int, comment: str):
    return await asyncio.to_thread(_set_feedback_comment_sync, telegram_id, event_id, comment)
...
def _checkin_by_token_sync(telegram_id: int, token: str, reward: int):
    with db_cursor() as (_connection, cursor):
        cursor.execute(
        ...
        cursor.execute(
            "UPDATE reg SET attended = 1, attended_at = NOW() WHERE id = %s",
            (reg["id"],),
        )
        if not reg.get("coins_awarded"):
            cursor.execute(
                "UPDATE reg SET coins_awarded = 1 WHERE id = %s AND coins_awarded = 0",
                (reg["id"],),
            )
            if cursor.rowcount == 1:
                cursor.execute(
                    "UPDATE users SET coins = COALESCE(coins, 0) + %s WHERE id = %s",
                    (reward, user["id"]),
                )
...
async def checkin_by_token(telegram_id: int, token: str, reward: int):
    return await asyncio.to_thread(_checkin_by_token_sync, telegram_id, token, reward)
